...
(function (window) {
    ...
    var global_timer = [];
    /**
     * 全局回调函数对象，用于不同的js库之间相互的函数调用
     * @property global_callbacks_object
     * @type object
     * @protected
     * @default {}
     */
    var global_callbacks_object = {};
    /**
     * 记录每个函数repeat的历史
     * @property repeat_history
     * @type object
     * @protected
     * @default {}
     */
    ...
    /**
     * 数据仓库的缓存对象
     * @property data_store_cache
     * @type object
     * @protected
     * @default {}
     */
    var data_store_cache = {};
    /**
     * 缓存监听数据仓库变化的回调函数集合的对象
     * @property data_store_change_listener
     * @type object
     * @protected
     * @default {}
     */
    var data_store_change_listener = {};
    /**
     * 等待需要触发的事件队列
     * @property event_waiting_list
     * @type object
     * @protected
     * @default []
     */
    var event_waiting_list = [];
    /**
     * 执行事件队列的定时器
     * @property fire_event_timer
     * @type object
     * @protected
     */
    var fire_event_timer = null;
    /**
     * 路由规则配置的缓存对象
     * @property route_config_cache
     * @type object
     * @protected
     * @default {}
     */
    var route_config_cache = {};
    ...
    var HiWiFi = {
         ...
         */
        isMobile: function () {
            return !!ua.match(/(Android|iPhone|SymbianOS|Windows Phone|iPod|MQQBrowser|XiaoMi)/gi);
        },
        /**
         * 通过UA判断浏览器是否位于app
         * @method isApp
         ...
         */
        isApp: function () {
            return !!ua.match(/HiWiFi/gi);
        },
        /**
         * 通过UA判断是否是苹果的设备(ios和mac os)
         * @method isAppleDevice
         * @public
         * @param
         * @return {boolean} 
         */
        isAppleDevice: function () {
            return !!ua.match(new RegExp(/\(i[^;]+;( U;)? CPU.+Mac OS X/gi));
        },
        /**
         * 通过UA判断是否是安卓
         * @method isAndroid
         ...
         */
        isAndroid: function () {
            return !!ua.match(new RegExp('Android', 'gi'));
        },
        /**
         * 获取chrome浏览器的版本号
         * @method getChromeNum
         ...
         * @return {number}  版本号
         */
        getChromeNum: function () {
            return HiWiFi.isArray(ua.match(/Chrome\/(\d+)/)) ? ua.match(/Chrome\/(\d+)/)[1] : 0;
        },
        /**
         * 获取IOS的版本号
         * @method getIOSNum
         ...
         */
        getIOSNum: function () {
            var ua_tmp = ua.toLowerCase();
            ua_tmp = ua_tmp.replace(/_/g, ".");
            ua_tmp = ua_tmp.match(/iphone os (\d+\.?\d*)/) || ua.match(/iphone os (\d+\.?\d*)/) || [];
            return parseFloat(ua_tmp[1] || 0, 10);
        },
        /**
         * 向浏览器设置cookie
         * @method setCookie
         ...
         * @param {number} [options]    过期时间（分），默认时间设置为120分钟
         * @return {string}             document.cookie字符串
         */
        ...
        getUrlHash: function (name) {
            ...
            if (name) {
                current_hash = reg.exec(current_hash) || [];
                current_hash = current_hash[0] || "";
                result = current_hash.replace(name + "=", "");
            }
            return result;
        },
        /**
        ...
        /**
         * 判断在数组中某字符是否存在 或 某数组是否被包含
         * @method inArray
         * @public
         * @param {string | array} arg_in       需要查找的数组或字符串
         * @param {array} arg_out               待处理数组
         * @param {boolean} [ignore_case]       是否忽略字符大小写
         * @return {boolean}
         */
        inArray: function (arg_in, arg_out, ignore_case) {
            var i = 0;
            if (!HiWiFi.isArray(arg_out)) {
                return false;
            } else if (HiWiFi.isArray(arg_in)) {
                if (arg_in.length > arg_out.length) {
                    return false;
                } else {
                    for (i in arg_in) {
                        if (!HiWiFi.inArray(arg_in[i], arg_out, ignore_case)) {
                            return false;
                        }
                    }
                    return true;
                }
            } else if (arg_in && typeof arg_in === "string") {
                for (i in arg_out) {
                    if (ignore_case) {
                        if (arg_in.toUpperCase() === arg_out[i].toUpperCase()) {
                            return true;
                        }
                    } else {
                        if (arg_in === arg_out[i]) {
                ...
                //数组里存储的为number、object、function...使用此方式判断
                for (i in arg_out) {
                    if (arg_in === arg_out[i]) {
                        return true;
                    }
            ...
            for (i = 0; i < names.length; i++) {
                ...
                if (HiWiFi.isArray(data_store_change_listener[name])) {
                    for (j in data_store_change_listener[name]) {
                        ...
                        //此处很重要，如果没有此功能的化，会产生大量的dom重复操作，很耗性能
